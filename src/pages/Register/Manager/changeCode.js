...
const ModalDelete = ({ show, onClose, password, setPassword, userId }) => {
  ...
  const handleChangePassword = async () => {
    try {
      if(!password || password.length < 3) {
        alert('O código deve ter no mínimo 3 caracteres');
        return;
      }
      setLoading(true);
      const { success } = (await Api.put(`/manager/updateManagerCode/${userId}`, {
        code: password
      })).data;
      if(success)
      ...
      alert('Sem permissão para mudar o código');
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={show} onClose={onClose} fullWidth maxWidth='md'>
      <DialogTitle>Mudança de código</DialogTitle>
      <DialogContent>

import { getBalanceTable, getDailySellsTable, getEventAndFilters, getProductsTable } from "./utils"
...
const content = (data) => {
  ...
  const balanceTable = getBalanceTable(resume)
  const dailySellsTable = getDailySellsTable(dailySells)
  const productsTable = getProductsTable(products)
    ...
    body: [
      {
        ...
        table: {
          ...
          body: [
            [
              {
                text: "Visão geral da Loja Virtual",
                style: "headerItem",
                fontSize: 11,
                bold: true,
              },

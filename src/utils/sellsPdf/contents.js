import { getEventAndFilters, getGroups, getTable } from "./utils"
...
const content = (data) => {
  const { event, operators, products, isAllGroups, dateIni, dateEnd, totals, isOverview, crudePays } =
    data

  const groupsStr = !isAllGroups ? getGroups(products).join(", ") : ["Todos"]
  ...
  const table = getTable(products, totals, isOverview, crudePays)
